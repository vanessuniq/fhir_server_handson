package org.saintmartinhospital.business.repository.impl;
...
import org.saintmartinhospital.business.domain.DocType_;
import org.saintmartinhospital.business.domain.Person;
import org.saintmartinhospital.business.domain.PersonDoc;
import org.saintmartinhospital.business.domain.PersonDoc_;
import org.saintmartinhospital.business.domain.Person_;
...
public class PersonDAOImpl implements PersonDAO {
	...
	public List<Person> findByCriteria( FindPersonByCriteriaBO criteria ) {
		...
		if( StringUtils.isNotEmpty( criteria.getLastName() ) ) {
			...
			predicates.add( builder.or(
				builder.equal( builder.lower( person.get( Person_.FATHERS_LAST_NAME ) ), lastNameParam ),
				builder.equal( builder.lower( person.get( Person_.MOTHERS_LAST_NAME ) ), lastNameParam )
			));
		}
		...
		if( criteria.getGender() != null ) {
			...
			predicates.add( builder.equal( builder.lower( person.get( Person_.GENDER ) ), builder.lower( builder.parameter( String.class, GENDER_PARAM ) ) ) );
		}
		
		// Build birth date criteria
		if( criteria.getBirthDate() != null ) {
			params.put( BIRTH_DATE_PARAM, criteria.getBirthDate() );
			predicates.add( builder.equal( person.get( Person_.BIRTH_DATE ), builder.parameter( Calendar.class, BIRTH_DATE_PARAM ) ) );
		}
		...
		if( StringUtils.isNotEmpty( criteria.getDocTypeAbrev() ) && StringUtils.isNotEmpty( criteria.getDocValue() ) ) {
			...
			predicates.add( builder.equal( builder.lower( personDoc.get( PersonDoc_.DOC_TYPE ).get( DocType_.ABREV ) ), docTypeAbrevParam ) );
